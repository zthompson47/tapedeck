...
";
    ...
    /*
fn main() {
    let playlist = parse(PLS).unwrap().1;
            ...
            if result.is_ok() {
                ...
                if result.is_ok() {
                    match result.unwrap().1 {
                        "numberofentries" => {
                            playlist.number_of_entries = right.parse::<u32>().unwrap();
                        },
                        "Version" => {
